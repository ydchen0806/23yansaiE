...
if __name__ == '__main__':
    ...
    cluster_score = []
    predict_score = []
    for feature_num in range(5,20,2):
        top_10_feature = feature_importance.iloc[:feature_num, 0].index.tolist()
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        best_mse = int(1e6)
        
        cluster = 4
        feature_num2.append(feature_num)
        ...
        data_in.to_excel(os.path.join(save_dir,f'q2_b_feauture_{feature_num}.xlsx'), index=False)
        cluster_center.to_excel(os.path.join(save_dir,f'q2_b_feauture_{feature_num}_center.xlsx'), index=False)
        print(f'feauter {feature_num} finished')
        ...
        if mse_error < best_mse:
            ...
            print(f'best feature_num is {feature_num}, best mse is {best_mse}')
        

        else:
            print(f'not best feature_num, best feature_num is {best_feature_num}, best mse is {best_mse}, current feature_num is {feature_num}, current mse is {mse_error}')
        record_dataframe.sort_values(by='ID', inplace=True)
        record_dataframe.to_excel(os.path.join(save_dir,f'q2_b_feature_{feature_num}_record.xlsx'), index=False)
        ...
        # plt.show()
        # plt.close()
    save_dict = {'特征数量':feature_num2, 'cluster_score': cluster_score, 'prediction score':predict_score}
    save_df = pd.DataFrame(save_dict)
    save_df.to_excel(os.path.join(save_dir,'特征数灵敏度分析.xlsx'))
